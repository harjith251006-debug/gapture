import { randomUUID } from "node:crypto";
import {
  COMPLIANCE_POLICIES_BUCKET,
  compliancePolicyPath,
  deriveKey,
  encrypt,
  sha256Hex,
} from "@gapture/shared";
import { ApiError, ok, requireOrganizationId, requireSession, route } from "@/lib/api";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * Company compliance policy upload + listing (PRD FR-12, plan Phase 8).
 *
 * POST: authenticated, org-scoped. Stores the AES-256-encrypted original at
 * compliance-policies/{organization_id}/{policy_id}/original.enc, dedupes on
 * (organization_id, sha256), and inserts an UPLOADED compliance_policies row.
 * The worker's policy sweep then does extract -> clean -> chunk -> embed.
 *
 * GET: lists the caller's organization's policies (RLS scopes the query).
 *
 * Deliberately NOT built (plan Phase 8 task 5): versioning, categorization,
 * approval workflow. "Replacing" a policy is just another upload.
 */

const ALLOWED_MIME_TYPES = new Set([
  "application/pdf",
  "text/plain",
  "text/markdown",
  "image/png",
  "image/jpeg",
]);
const MAX_UPLOAD_BYTES = 10 * 1024 * 1024;
/** OCR.space free tier caps PDFs/images at 1 MB — warn, don't block. */
const OCR_SOFT_LIMIT_BYTES = 1024 * 1024;

export const POST = route("api/policies", async (req) => {
  const session = await requireSession();
  const organizationId = await requireOrganizationId(session);

  const encryptionKey = process.env.DOCUMENT_ENCRYPTION_KEY;
  if (!encryptionKey) {
    console.error(JSON.stringify({ level: "error", component: "api/policies", message: "DOCUMENT_ENCRYPTION_KEY not set" }));
    throw new ApiError("internal", "Policy storage is not configured on this server");
  }

  let formData: FormData;
  try {
    formData = await req.formData();
  } catch {
    throw new ApiError("bad_request", "Expected multipart/form-data with a 'file' field");
  }

  const file = formData.get("file");
  if (!(file instanceof File)) throw new ApiError("bad_request", "A 'file' field is required");
  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    throw new ApiError(
      "unsupported_media_type",
      `Unsupported file type '${file.type || "unknown"}'. Allowed: PDF, plain text, Markdown, PNG, JPEG.`,
    );
  }
  if (file.size > MAX_UPLOAD_BYTES) {
    throw new ApiError("payload_too_large", `File is ${file.size} bytes, over the ${MAX_UPLOAD_BYTES}-byte limit`);
  }

  const titleRaw = formData.get("title");
  const title = (typeof titleRaw === "string" && titleRaw.trim()) || file.name || "Untitled policy";

  const bytes = Buffer.from(await file.arrayBuffer());
  const sha256 = sha256Hex(bytes);

  const { data: existing } = await session.supabase
    .from("compliance_policies")
    .select("id")
    .eq("organization_id", organizationId)
    .eq("sha256", sha256)
    .maybeSingle();
  if (existing) throw new ApiError("conflict", "This exact file is already uploaded for your organization");

  const policyId = randomUUID();
  const storagePath = compliancePolicyPath(organizationId, policyId, "original.enc");
  const ciphertext = encrypt(bytes, deriveKey(encryptionKey));

  const { error: uploadError } = await session.supabase.storage
    .from(COMPLIANCE_POLICIES_BUCKET)
    .upload(storagePath, ciphertext, { contentType: "application/octet-stream", upsert: false });
  if (uploadError) {
    console.error(JSON.stringify({ level: "error", component: "api/policies", message: "storage upload failed", error: uploadError.message }));
    throw new ApiError("internal", "Failed to store the uploaded file");
  }

  const { data: inserted, error: insertError } = await session.supabase
    .from("compliance_policies")
    .insert({
      id: policyId,
      organization_id: organizationId,
      title,
      storage_bucket: COMPLIANCE_POLICIES_BUCKET,
      storage_path: storagePath,
      mime_type: file.type,
      file_size_bytes: bytes.byteLength,
      sha256,
      status: "UPLOADED",
    })
    .select("id, title, status, created_at")
    .single();

  if (insertError || !inserted) {
    await session.supabase.storage.from(COMPLIANCE_POLICIES_BUCKET).remove([storagePath]);
    console.error(JSON.stringify({ level: "error", component: "api/policies", message: "policy insert failed", error: insertError?.message }));
    throw new ApiError("internal", "Failed to record the policy");
  }

  const overOcrLimit =
    bytes.byteLength > OCR_SOFT_LIMIT_BYTES && file.type !== "text/plain" && file.type !== "text/markdown";

  return ok(
    {
      id: inserted.id,
      title: inserted.title,
      status: inserted.status,
      createdAt: inserted.created_at,
      warning: overOcrLimit
        ? "File is over OCR.space's 1 MB free-tier limit; text extraction may fail until a paid OCR tier is configured."
        : null,
    },
    undefined,
    { status: 201 },
  );
});

export const GET = route("api/policies", async () => {
  const { supabase } = await requireSession();

  const { data, error } = await supabase
    .from("compliance_policies")
    .select("id, title, status, created_at")
    .order("created_at", { ascending: false });

  if (error) throw new Error(`policy list failed: ${error.message}`);

  return ok(
    (data ?? []).map((p) => ({ id: p.id, title: p.title, status: p.status, createdAt: p.created_at })),
  );
});
